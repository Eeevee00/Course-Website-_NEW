...
class Ion_auth
{
	...
	public function forgotten_password($identity)
	{
		...
		$user = $this->where($this->Auth_model->identity_column, $identity)
					 ->where('active', 1)
					 ->users()->row();
		...
		{
			// Generate code
			$code = $this->Auth_model->forgotten_password($identity);
	...
	{
		$user = $this->Auth_model->get_user_by_forgotten_password_code($code);
		...
		else
		{
			if ($this->config->item('forgot_password_expiration', 'ion_auth') > 0)
			{
				...
				if (time() - $user->forgotten_password_time > $expiration)
				{
					...
					$this->Auth_model->clear_forgotten_password_code($identity);
					$this->set_error('password_change_unsuccessful');
					return FALSE;
				}
			}
	...
	public function logout()
	{
		...
		// Clear all codes
		// $this->Auth_model->clear_forgotten_password_code($identity);
		// $this->Auth_model->clear_remember_code($identity);
	...
	public function is_admin($id = FALSE)
	{
		...
		return $this->Auth_model->in_group($admin_group, $id);
	}
	...
	protected function check_compatibility()
	{
		...
		// See functions used in Auth_model::_random_token()
	...
	public function deactivate($id = NULL)
	{
		...
		return $this->Auth_model->deactivate($id);
	}
